...
- **MCP サーバー** (`src/index.ts`) — Claude Desktop 等の MCP クライアント向け（stdio トランスポート、ツール `generate_video` / `edit_video` / `extend_video` / `upload_file`）
...
このフロー全体（API 呼び出し→HTTPエラーの McpError 変換→ポーリング→ダウンロード→結果構築）は `src/utils/video.ts` の `runVideoJob` に共通化されており、`generateVideo` / `editVideo` / `extendVideo` は「バリデーション＋リクエストボディ構築＋`runVideoJob` 呼び出し」の薄いラッパー。MCP 応答テキストの整形も同ファイルの `formatVideoResult` に共通化。`pollInterval` と `maxPollAttempts` は引数で受け取り、MCP・CLI 双方から制御される。429 エラーメッセージには文字列 `429` を必ず含めること（バッチの `retry_on_errors` が部分一致で判定するため）。
...
- ジョブ種別は `getJobOperation` + `isImageToVideo` / `isReferenceToVideo`（`src/utils/batch-config.ts`）で判定：`operation` 明示が最優先、なければソース動画（`video_url` / `video_file_id`）あり → edit、それ以外は generate。generate 内で `reference_images` → R2V、`image_*` → I2V。延長は `"operation": "extend"` を明示（ソース動画だけだと edit 扱い）。編集・延長のソース動画は `video_url` / `video_path` / `video_file_id` のいずれか1つ（相互排他）。
...
- `estimateBatchCost` のコスト単価（`MODEL_COSTS_PER_SECOND`）は**モデル別の公式料金**（2026-07 時点: `grok-imagine-video` $0.050/秒、`grok-imagine-video-1.5` $0.080/秒）。実課金額はジョブごとに `usage.cost_in_usd_ticks` で返る。
- バッチ設定の読み込み・検証・マージは `src/utils/batch-config.ts`（`loadBatchConfig` / `validateBatchConfig` / `mergeBatchConfig`、CLI フラグが設定ファイルより優先）。

### ローカル画像の base64 変換

`image_path` や `reference_images[].path` を指定するとローカルファイルを読み込んで **base64 data URL** に変換し API へ送る（`src/tools/generate.ts` の `resolveLocalImage` ヘルパに集約）。以前は Cloudflare R2 アップロードだったが廃止済み。`MAX_IMAGE_FILE_BYTES`（10MB、`src/types/tools.ts`）を超える画像は base64 の代わりに **xAI Files API へ自動アップロード**され `file_id` で参照される（Files API の上限は `MAX_UPLOAD_FILE_BYTES` = 48MB）。拡張子は `IMAGE_MIME_TYPES`（`src/utils/files.ts`）にあるもののみ（未知の拡張子はクライアント側で即エラー。MIME 宣言がペイロード側を伝搬するため octet-stream フォールバックは不可）。I2V のソース画像は `image_url` / `image_path` / `image_file_id` のいずれか1つ（相互排他）。画像（I2V）と `reference_images`（R2V）も排他。`prompt` は T2V/R2V では必須だが I2V（画像指定時）では省略可。

### Files API 連携 (`src/utils/files.ts`)

`uploadFileToXai` が `POST /v1/files`（multipart、`purpose: assistants`）でアップロードし `file_id` を返す。利用箇所は3つ：(1) MCP ツール `upload_file`（任意の画像/動画を明示アップロード）、(2) 10MB 超のローカル画像の自動アップロード、(3) `video_path`（edit/extend のローカル動画、`resolveLocalVideo`、.mp4 のみ）。編集・延長のソース動画は `video_url` / `video_path` / `video_file_id` のいずれか1つ（相互排他）。
...
- `1080p` は **`grok-imagine-video-1.5` × I2V の組み合わせに限り利用可能**（公式ドキュメント記載・2026-07-11 実機検証済み。出力実寸は 1904x1072）。それ以外のモデル×モードでは `1080p video resolution is not available for this model.` が返る。アカウント権限や Grok サブスク（API課金とは別系統）とは無関係。エラー本文は `extractApiErrorMessage`（`src/utils/video.ts`）で文字列/オブジェクト両形式から抽出して表示する。
