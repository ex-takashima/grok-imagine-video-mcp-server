...
`image_path` や `reference_images[].path` を指定するとローカルファイルを読み込んで **base64 data URL** に変換し API へ送る（`src/tools/generate.ts` の `imagePathToDataUrl` ヘルパに集約）。以前は Cloudflare R2 アップロードだったが廃止済み。ファイルサイズ上限は `MAX_IMAGE_FILE_BYTES`（10MB、`src/types/tools.ts`）、拡張子は `IMAGE_MIME_TYPES` にあるもののみ（未知の拡張子はクライアント側で即エラー。data URL では MIME 宣言がペイロード内を伝搬するため octet-stream フォールバックは不可）。I2V のソース画像は `image_url` / `image_path` / `image_file_id`（xAI Files API の File ID）のいずれか1つ（相互排他）。画像（I2V）と `reference_images`（R2V）も排他。`prompt` は T2V/R2V では必須だが I2V（画像指定時）では省略可。
