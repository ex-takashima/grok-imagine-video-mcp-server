...
- `1080p` は **xAI の動画 API が現状提供していない**（公式ドキュメントの解像度は `480p`/`720p` のみで 720p が上限。1080p はロードマップ）。どのモデル×モードでも `1080p video resolution is not available for this model.` が返る。アカウント権限や Grok サブスク（API課金とは別系統）とは無関係。`RESOLUTIONS` に `1080p` を残してあるのは将来対応時の前方互換のため。エラー本文は `extractApiErrorMessage`（`src/utils/video.ts`）で文字列/オブジェクト両形式から抽出して表示する。
